...
import {
  ...
  NodeDependencyType,
} from "@nestjs/schematics/dist/utils/dependencies.utils";
import { normalizeToKebabOrSnakeCase } from "@nestjs/schematics/dist/utils/formatting";
import {
  Location,
  NameParser,
} from "@nestjs/schematics/dist/utils/name.parser";
...
function generate(options: ResourceOptions): Source {
  return (context: SchematicContext) =>
    apply(url(join("./files" as Path, options.language as any)), [
      filter((path: any): boolean => {
        ...
        if (path.endsWith(".graphql")) {
          return (
            (options.type === "graphql-schema-first" && options.crud) || false
          );
        }
        if (
